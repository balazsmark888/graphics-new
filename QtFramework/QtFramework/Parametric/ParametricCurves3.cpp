...
ParametricCurve3::ParametricCurve3(const RowMatrix<Derivative>& derivatives, GLdouble u_min, GLdouble u_max) :
	_u_min(u_min), _u_max(u_max), _derivatives(derivatives), firstDerivScale(1), secondDerivScale(1)
{
}

ParametricCurve3::ParametricCurve3(const RowMatrix<Derivative>& derivatives, GLdouble u_min, GLdouble u_max, GLdouble firstDerivScale, GLdouble secondDerivScale) :
	_u_min(u_min), _u_max(u_max), _derivatives(derivatives), firstDerivScale(firstDerivScale), secondDerivScale(secondDerivScale)
{
}
...
DCoordinate3 ParametricCurve3::operator ()(GLuint order, GLdouble u) const
{
	...
	{
	case 1:
		return _derivatives[order](u) * firstDerivScale;
	case 2:
		return _derivatives[order](u) * secondDerivScale;
	default:
...
GenericCurve3* ParametricCurve3::GenerateImage(GLuint div_point_count, GLenum usage_flag) const
{
	...
	for (GLuint i = 1; i < div_point_count - 1; i++)
	{
		...
		for (GLuint order = 0; order < _derivatives.GetColumnCount(); ++order)
		{
			...
			{
			case 1:
				(*result)(order, i) = _derivatives[order](u) * firstDerivScale;
				break;
			case 2:
				(*result)(order, i) = _derivatives[order](u) * secondDerivScale;
				break;
			default:
