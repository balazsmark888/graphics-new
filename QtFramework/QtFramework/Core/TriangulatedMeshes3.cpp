...
GLboolean TriangulatedMesh3::LoadFromOFF(const string& file_name, GLboolean translate_and_scale_to_unit_cube)
{
...
GLboolean cagd::TriangulatedMesh3::LoadFromFstream(fstream& f, GLboolean translate_and_scale_to_unit_cube)
{
	GLuint vertex_count, face_count, edge_count;
...
GLboolean cagd::TriangulatedMesh3::SaveToOFF(const std::string& file_name) const
{
	...
	// saving faces
	for (auto fit = _face.begin(); fit != _face.end(); ++fit)
		f << *fit << endl;
...
GLboolean cagd::TriangulatedMesh3::SaveToFstream(fstream& f) const
{
	// saving number of vertices, faces, and edges
	GLuint vertex_count, face_count, edge_count;
...
GLvoid cagd::TriangulatedMesh3::Translate(GLdouble translateX, GLdouble translateY, GLdouble translateZ)
{
	for (auto iterator = _vertex.begin(); iterator != _vertex.end(); ++iterator)
	{
		(*iterator).setx((*iterator).x() + translateX);
		(*iterator).sety((*iterator).y() + translateY);
		(*iterator).setz((*iterator).z() + translateZ);
	}
}

GLvoid cagd::TriangulatedMesh3::Scale(GLdouble scale)
{
	for (auto iterator = _vertex.begin(); iterator != _vertex.end(); ++iterator)
	{
		(*iterator).setx((*iterator).x() * scale);
		(*iterator).sety((*iterator).y() * scale);
		(*iterator).setz((*iterator).z() * scale);
	}
}

GLvoid cagd::TriangulatedMesh3::Rotate(GLdouble rotateX, GLdouble rotateY, GLdouble rotateZ)
{
	auto sinX = sin(rotateX * PI / 180);
	auto cosX = cos(rotateX * PI / 180);
	auto sinY = sin(rotateY * PI / 180);
	auto cosY = cos(rotateY * PI / 180);
	auto sinZ = sin(rotateZ * PI / 180);
	auto cosZ = cos(rotateZ * PI / 180);
	for (auto iterator = _vertex.begin(); iterator != _vertex.end(); ++iterator)
	{
		auto x = (*iterator).x();
		auto y = (*iterator).y();
		...
		//x
		(*iterator).sety(y * cosX - z * sinX);
		(*iterator).setz(z * cosX + y * sinX);
		...
		//y
		(*iterator).setx(x * cosY + z * sinY);
		(*iterator).setz(z * cosY - x * sinY);
		...
		////z
		//(*iterator).setx(x * cosZ - y * sinZ);
		//(*iterator).sety(y * cosZ + x * sinZ);
	}
}
