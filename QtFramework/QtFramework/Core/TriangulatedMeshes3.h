...
namespace cagd
{
	class TriangulatedMesh3
	{
		...
		friend std::ostream& operator<<(std::ostream& lhs, const TriangulatedMesh3& rhs)
		{
			...
			for (std::vector<DCoordinate3>::const_iterator vit = rhs._vertex.begin();
				vit != rhs._vertex.end();
				++vit) {
				lhs << *vit << std::endl;
			}
			for (std::vector<DCoordinate3>::const_iterator vit = rhs._normal.begin();
				vit != rhs._normal.end();
				++vit) {
				lhs << *vit << std::endl;
			}
			for (std::vector<TCoordinate4>::const_iterator vit = rhs._tex.begin();
				vit != rhs._tex.end();
				++vit) {
				lhs << *vit << std::endl;
			}
			for (std::vector<TriangularFace>::const_iterator vit = rhs._face.begin();
				vit != rhs._face.end();
				++vit) {
				lhs << *vit << std::endl;
			}
		...
		friend std::istream& operator>>(std::istream& lhs, TriangulatedMesh3& rhs)
		{
			...
			rhs._tex.resize(num);
			lhs >> num;
			rhs._face.resize(num);

			for (std::vector<DCoordinate3>::iterator vit = rhs._vertex.begin();
				vit != rhs._vertex.end();
				++vit) {
				lhs >> (*vit);
			}
			for (std::vector<DCoordinate3>::iterator vit = rhs._normal.begin();
				vit != rhs._normal.end();
				++vit) {
				lhs >> *vit;
			}
			for (std::vector<TCoordinate4>::iterator vit = rhs._tex.begin(); vit != rhs._tex.end();
				++vit) {
				lhs >> *vit;
			}
			for (std::vector<TriangularFace>::iterator vit = rhs._face.begin();
				vit != rhs._face.end();
				++vit) {
				lhs >> *vit;
			}
	...
	public:
		...
		GLboolean LoadFromFstream(std::fstream& f, GLboolean translate_and_scale_to_unit_cube = GL_FALSE);

		// homework: saves the geometry into an OFF file
		GLboolean SaveToOFF(const std::string& file_name) const;
		GLboolean SaveToFstream(std::fstream& f) const;
		...
		GLvoid Translate(GLdouble translateX, GLdouble translateY, GLdouble translateZ);
		GLvoid Scale(GLdouble scale);
		GLvoid Rotate(GLdouble rotateX, GLdouble rotateY, GLdouble rotateZ);
