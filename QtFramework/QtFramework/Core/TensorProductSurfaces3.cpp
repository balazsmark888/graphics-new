...
GLboolean TensorProductSurface3::SetData(
	GLuint row, GLuint column, GLdouble x, GLdouble y, GLdouble z)
{
...
GLboolean TensorProductSurface3::GetData(
	GLuint row, GLuint column, GLdouble& x, GLdouble& y, GLdouble& z) const
{
	if (row >= _data.GetRowCount() || column >= _data.GetColumnCount()) {
		return GL_FALSE;
	}
...
{
	if (!_vbo_data
		|| (render_mode != GL_LINE_STRIP && render_mode != GL_LINE_LOOP
...
GLboolean TensorProductSurface3::UpdateVertexBufferObjectsOfData(GLenum usage_flag)
{
	...
	if (usage_flag != GL_STREAM_DRAW && usage_flag != GL_STREAM_READ && usage_flag != GL_STREAM_COPY
		...
		&& usage_flag != GL_DYNAMIC_COPY && usage_flag != GL_STATIC_DRAW
